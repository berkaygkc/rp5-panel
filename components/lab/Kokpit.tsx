...
type AppId = "claude" | "infra" | "mail" | "chat" | "media" | "shortcuts";
...
export default function Kokpit({ model, gear }: { model: LabModel; gear: Gear }) {
  const [focused, setFocus] = useState<AppId | null>(null);
  const [sub, setSub] = useState<string | null>(null);
  // Sakin viteste odak düşer: ekran kendiliğinden dinlenmeye döner
  const focus = gear === "calm" ? null : focused;
  ...
  const active = APPS.find((a) => a.id === focus) ?? null;

  return (
    <div className="k4-root" style={{ ["--k4-hue" as string]: active?.hue ?? "220 20% 60%" }}>
      <div className="k4-aurora" aria-hidden />

      <aside className="k4-spine">
        <div className="k4-time">{fmtTime(os.now)}</div>
        <div className="k4-date">{fmtDate(os.now)}</div>
        <div className="k4-lamps">
        ...
        <div className="k4-verdict">
          {model.criticals > 0 ? `${model.criticals} sorun` : model.waiting > 0 ? `${model.waiting} bekliyor` : "sakin"}
        </div>
      </aside>
...
function Compact({
  ...
  const top = [...shortcuts].sort((a, b) => (b.lastRunAt ?? 0) - (a.lastRunAt ?? 0)).slice(0, 4);
  return (
    <div className="k4-c">
...
const fmtTime = (ms: number) => (ms ? new Date(ms).toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" }) : "--:--");
const fmtClock = (ms: number) => new Date(ms).toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" });
const fmtDate = (ms: number) => (ms ? new Date(ms).toLocaleDateString("tr-TR", { day: "numeric", month: "long" }) : "");
